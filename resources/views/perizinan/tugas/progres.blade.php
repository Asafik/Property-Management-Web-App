...
@section('title', 'Update Progres Tugas Perizinan - Property Management App')
                ...
                <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <small class="text-muted" style="font-size: 0.78rem;">Pembaruan data otomatis tercatat ke dalam Audit Trail atas nama <strong>{{ auth()->user()->name }}</strong></small>
                        </div>
                    </div>
                </div>

                <div class="card-body">
                    <form action="{{ route('perizinan.tugas.progress', $task->id) }}" method="POST" enctype="multipart/form-data" id="formUpdateProgresPage">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <div class="row g-2">
                                    <div class="col-6 col-md-3">
                                        <label class="status-radio-card {{ old('status', $task->status) == 'Dalam Proses' ? 'active' : '' }}" id="cardStatusProses">
                                            <input type="radio" name="status" value="Dalam Proses" {{ old('status', $task->status) == 'Dalam Proses' ? 'checked' : '' }} onchange="onStatusRadioChange('Dalam Proses')">
                                            <div>
                                    ...
                                    <div class="col-6 col-md-3">
                                        <label class="status-radio-card {{ old('status', $task->status) == 'Selesai' ? 'active' : '' }}" id="cardStatusSelesai">
                                            <input type="radio" name="status" value="Selesai" {{ old('status', $task->status) == 'Selesai' ? 'checked' : '' }} onchange="onStatusRadioChange('Selesai')">
                                            <div>
                                    ...
                                    <div class="col-6 col-md-3">
                                        <label class="status-radio-card {{ old('status', $task->status) == 'Terkendala' ? 'active' : '' }}" id="cardStatusTerkendala">
                                            <input type="radio" name="status" value="Terkendala" {{ old('status', $task->status) == 'Terkendala' ? 'checked' : '' }} onchange="onStatusRadioChange('Terkendala')">
                                            <div>
                                    ...
                                    <div class="col-6 col-md-3">
                                        <label class="status-radio-card {{ old('status', $task->status) == 'Pending' ? 'active' : '' }}" id="cardStatusPending">
                                            <input type="radio" name="status" value="Pending" {{ old('status', $task->status) == 'Pending' ? 'checked' : '' }} onchange="onStatusRadioChange('Pending')">
                                            <div>
                            ...
                            <div class="col-12">
                                <div class="d-flex align-items-center justify-content-between mb-1">
                                    ...
                                    <div class="d-flex align-items-center gap-1">
                                        <span class="badge bg-primary px-2 py-1 fs-6 fw-bold" id="badgeProgressVal" style="border-radius: 4px;">{{ old('progress', $task->progress) }}%</span>
                                    </div>
                                </div>

                                <div class="d-flex align-items-center gap-3">
                                    <input type="range" class="form-range flex-grow-1" id="inputProgressRange" min="0" max="100" step="5" value="{{ old('progress', $task->progress) }}" oninput="syncProgressInput(this.value)">
                                    <input type="number" name="progress" id="inputProgressNumber" class="form-control form-control-custom text-center fw-bold" style="width: 80px;" min="0" max="100" value="{{ old('progress', $task->progress) }}" oninput="syncProgressRange(this.value)" required>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nomor_dokumen" id="inputNomorDokumen" class="form-control form-control-custom" placeholder="Contoh: 503/124/DPMPTSP/2026" value="{{ old('nomor_dokumen', $task->nomor_dokumen) }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="tanggal_terbit" id="inputTanggalTerbit" class="form-control form-control-custom" value="{{ old('tanggal_terbit', $task->tanggal_terbit ? $task->tanggal_terbit->format('Y-m-d') : '') }}">
                                <small class="text-muted d-block mt-1" style="font-size: 0.74rem;">
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex flex-wrap align-items-center justify-content-between mt-1">
                                    ...
                                    @if($task->file_dokumen)
                                        <a href="{{ asset('storage/' . $task->file_dokumen) }}" target="_blank" class="text-primary fw-semibold d-inline-flex align-items-center gap-1" style="font-size: 0.78rem;">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="kendala" rows="4" class="form-control form-control-custom" placeholder="Tuliskan detail tindak lanjut hari ini, berkas yang kurang, respon dinas, atau kendala yang dihadapi...">{{ old('kendala', $task->kendala) }}</textarea>
                                <small class="text-muted d-block mt-1" style="font-size: 0.74rem;">
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body p-3">
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-6">
                            ...
                            <div class="info-value" style="font-size: 0.84rem;">
                                {{ $task->proyek_nama ?: ($task->proyek->land_name ?? 'Umum / Bebas') }}
                            </div>
                        </div>
                        <div class="col-6">
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-6">
                            ...
                            <div class="info-value" style="font-size: 0.84rem;">
                                {{ $task->employee->name ?? 'Belum Ditugaskan' }}
                            </div>
                            <small class="text-muted" style="font-size: 0.7rem;">{{ $task->employee->position->name ?? 'Staf Legal' }}</small>
                        </div>
                        <div class="col-6">
                            ...
                            @if($task->deadline)
                                @if(now()->startOfDay()->gt($task->deadline->startOfDay()) && $task->status !== 'Selesai')
            ...
            <div class="card compact-table-card w-100">
                <div class="card-header py-2.5 px-3 d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge bg-light text-muted border" style="font-size: 0.7rem;">{{ count($recentLogs) }} log</span>
                </div>
                <div class="card-body p-3">
                            ...
                            @foreach($recentLogs as $log)
                                <div class="timeline-mini-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        <span class="fw-bold text-dark" style="font-size: 0.78rem;">{{ $log->action }}</span>
                                        <small class="text-muted" style="font-size: 0.68rem;">{{ $log->created_at->diffForHumans() }}</small>
                                    </div>
                                    <div class="text-muted" style="font-size: 0.73rem; line-height: 1.3;">
                                        {{ Str::limit($log->keterangan, 90) }}
                                    </div>
                                    <div class="d-flex align-items-center gap-1 mt-0.5">
                                        <small class="fw-semibold text-primary" style="font-size: 0.68rem;">{{ $log->user->name ?? 'User' }}</small>
                                        @if($log->new_progress !== null)
                                            <span class="badge bg-light text-dark border py-0 px-1" style="font-size: 0.65rem;">{{ $log->new_progress }}%</span>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
