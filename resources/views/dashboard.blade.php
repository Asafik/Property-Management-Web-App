...
.stat-card .stat-icon.proyek     { background: linear-gradient(135deg, #667eea, #764ba2); }
.stat-card .stat-icon.unit       { background: linear-gradient(135deg, #f093fb, #f5576c); }
.stat-card .stat-icon.transaksi  { background: linear-gradient(135deg, #4facfe, #00f2fe); }
.stat-card .stat-icon.pendapatan { background: linear-gradient(135deg, #43e97b, #38f9d7); }
.stat-card .stat-content { flex: 1; min-width: 0; }
.stat-card .stat-content h3 {
    font-size: 1.8rem; font-weight: 700; margin-bottom: 0.2rem;
    color: #2c2e3f; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.stat-card .stat-content p {
    font-size: 0.9rem; color: #6c757d; margin-bottom: 0;
...
.badge {
    ...
    border-radius: 30px; display: inline-block; white-space: nowrap;
}
...
.table thead th {
    ...
    border-bottom: 2px solid #e9ecef; padding: 0.8rem 0.5rem; white-space: nowrap;
    cursor: pointer;
...
.text-price { color: #28a745 !important; font-weight: 600; }
...
@media (max-width: 1200px) {
    .location-wrapper { max-width: 180px; }
}

@media (max-width: 992px) {
    .location-wrapper { max-width: 150px; }
}

@media (max-width: 768px) {
    .location-wrapper { max-width: 120px; }
}

@media (max-width: 576px) {
    .location-wrapper { max-width: 100px; }
}
...
.table tbody td i { font-size: 1rem; }
                ...
                <div class="card-body">
                            ...
                            <div class="filter-row-desktop">
                                <div class="row g-2 align-items-end w-100">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" class="form-control" id="searchInput" placeholder="Nama proyek..." value="{{ request('search') }}">
                                    </div>
                            ...
                            <div class="filter-row-mobile">
                                <div class="row g-2">
                                    <div class="col-12 mb-2">
                                        <label class="form-label">Cari</label>
                                        <input type="text" class="form-control" id="searchInputMobile" placeholder="Nama proyek..." value="{{ request('search') }}">
                                    </div>
                                    <div class="col-12 mb-2">
                                        <label class="form-label">Perusahaan</label>
                                        <select class="form-control select2-mobile" id="perusahaanSelectMobile" style="width: 100%;">
                                            <option value="">Semua Perusahaan</option>
                                            @foreach($filterOptions['perusahaan'] ?? [] as $company)
                                                <option value="{{ $company }}" {{ request('perusahaan') == $company ? 'selected' : '' }}>{{ $company }}</option>
                                            @endforeach
                                    ...
                                    <div class="col-6">
                                        <button type="button" class="btn btn-gradient-secondary btn-icon-only-mobile w-100" id="resetBtnMobile" title="Reset">
                                            <i class="mdi mdi-refresh"></i>
                                        </button>
                                    </div>
                                </div>
                        ...
                        <table class="table table-hover align-middle" id="proyekTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sortable" data-field="zoning" data-direction="{{ request('sortField') == 'zoning' ? (request('sortDirection') == 'asc' ? 'desc' : 'asc') : 'asc' }}">
                                        Type
                                    ...
                                    <th class="sortable" data-field="acquisition_price" data-direction="{{ request('sortField') == 'acquisition_price' ? (request('sortDirection') == 'asc' ? 'desc' : 'asc') : 'asc' }}">
                                        Harga Diperoleh
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <i class="mdi mdi-city text-primary me-2" style="font-size: 1.2rem;"></i>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center location-wrapper">
                                            <i class="mdi mdi-map-marker text-primary me-2" style="font-size: 1.2rem; flex-shrink: 0;"></i>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="status-badge-gradient warning">
                                                <i class="mdi mdi-clock"></i> {{ ucfirst($item->status ?? '-') }}
                                            </span>
                                        @endif
                                    </td>
                                    <td class="text-price">
                                    ...
                                    <td class="text-center">
                                        <button class="btn btn-outline-purple btn-sm" title="Detail"
                    ...
                    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center mt-4">
                        <div class="pagination-info mb-2 mb-sm-0">
                            Menampilkan {{ $landBank->firstItem() }} - {{ $landBank->lastItem() }} dari {{ $landBank->total() }} data
                        </div>
                        <nav aria-label="Page navigation">
                            <ul class="pagination pagination-sm flex-wrap justify-content-center mb-0">
                                ...
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $landBank->appends(request()->query())->previousPageUrl() }}" rel="prev" aria-label="Previous">
                                            <i class="mdi mdi-chevron-left"></i>
                                        </a>
                                    </li>
                                @endif
                                ...
                                @foreach ($landBank->getUrlRange(max(1, $landBank->currentPage() - 2), min($landBank->lastPage(), $landBank->currentPage() + 2)) as $page => $url)
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $landBank->appends(request()->query())->url($page) }}">{{ $page }}</a>
                                        </li>
                                    @endif
                                @endforeach
                                ...
                                @if ($landBank->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $landBank->appends(request()->query())->nextPageUrl() }}" rel="next" aria-label="Next">
...
<div class="modal fade" id="modalDetailLandbank{{ $item->id }}" tabindex="-1" aria-labelledby="modalDetailLabel{{ $item->id }}" aria-hidden="true">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                @else
                                    <span class="status-badge-gradient warning">{{ ucfirst($item->status ?? '-') }}</span>
                                @endif
                            </div>
                            <div class="col-12">
                                    ...
                                    <tr>
                                            ...
                                            @if($unit->construction_progress)
                                                @switch($unit->construction_progress)
                                                    ...
                                                    @case('selesai')
                                                        ...
                                                        @break
                                                    @default
                                                        <span class="badge bg-secondary">{{ ucfirst($unit->construction_progress) }}</span>
                                                @endswitch
                                            @else
                                        ...
                                        <td>
                                            ...
                                            @elseif($unit->status == 'dipesan' || $unit->status == 'booked')
                                                <span class="badge" style="background: #ffc107; color: #2c2e3f;">
